...
async def test_1():
    ...
    description = """
      ...
      If asked, child has no chest indrwaring or stridor.
    ...
    result = await scenario.run(
        name="OneDay - 4yo with cough and fever",
...
async def test_2():
    ...
    criteria = [
        ...
        "Agent doesn't ask for the same information multiple times",
        "Agent asks correct follow-up questions according to the OneDay medical guidelines",
        "Agent provides the following diagnosis from the OneDay guidelines: Treat and communicate as uncomplicated Typhoid fever"
    ]

    result = await scenario.run(
        name="OneDay - 35yo woman with fever",
        description=description,
        agents=[
            OneDayAgent(),
            scenario.UserSimulatorAgent(),

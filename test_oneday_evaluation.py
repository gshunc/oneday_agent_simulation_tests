...
async def run_oneday_scenario(test_scenario: Scenario, testrun_uid: str, model_id: str, diagnosis_only: bool = False, use_turn: bool = False, turn_uuid: str | None = None, request=None):
    ...
    nurse_description = (
        ...
        "Turn 2 (after the agent greets you): Provide EXACTLY what's in the case scenario as "
        "the first nurse message, directly quoting from the scenario text. Obviously, don't tell "
        "the agent that you're doing so. For example, in a case you'll get a script. It will look "
        "like this: {\"name\": \"A 55-year-old man with systolic blood pressure 165 and mild "
        "headache\", \"description\": \"NURSE: A 55-year-old man with a blood pressure of 165 "
        "systolic and a very mild headache but no other symptoms. RDT for malaria was negative "
        "and his temperature was 36.8\u00b0C. What should I do next? ONEDAY_AGENT_QUESTIONS: "
        "- Has their blood pressure been tested before? (NURSE_RESPONSE: no) - Have you tested "
        "blood sugar as well to check for diabetes? (NURSE_RESPONSE: yes its 6.0 fasting) - Are "
        "they a smoker (NURSE_RESPONSE: no) AGENT_ANSWER: - With only one measurement high, "
        "there should be another test on another day before starting medication, so ask them to "
        "come back. DIAGNOSIS: Tension headache OR no diagnosis\", \"expected_diagnosis\": "
        "\"Tension headache OR no diagnosis\", \"case_number\": 1}. Provide the case by case "

...
def oneday_judge_prompt(scenario_description: str, criteria: list[str]) -> str:
    ...
    return f"""
      ...
      - IMPORTANT — Diagnosis matching must be SEMANTICALLY FLEXIBLE, not a literal string match. When evaluating whether the agent's diagnosis matches the expected diagnosis, apply ALL of the following leniencies:
        1. **Qualifier tolerance**: If the expected diagnosis includes a stage, type, or severity qualifier (e.g. "secondary syphilis", "severe malaria", "diabetes mellitus type 1", "possible pulmonary tuberculosis") and the agent correctly identifies the base condition (e.g. "syphilis", "malaria", "diabetes mellitus", "tuberculosis"), count it as correct — especially if the agent's treatment/management aligns with the qualified form.
        2. **Synonym and alternate phrasing**: Accept common synonyms, alternate medical names, and spelling variations for the same condition (e.g. "pink eye" = "conjunctivitis", "chickenpox" = "varicella", "TB" = "tuberculosis", "diarrhoea" = "diarrhea").
        3. **Word order and framing**: The components of a compound diagnosis may appear in any order. E.g. "Suspected meningitis with MRDT-positive malaria" is equivalent to "Severe malaria with suspected meningitis." Similarly, "suspected X" and "possible X" are interchangeable qualifiers.
        4. **Severity/subtype inferred from management**: If the agent's stated management clearly corresponds to a specific severity or subtype (e.g. treating with artesunate = severe malaria, referring urgently for Type 1 diabetes), the diagnosis is correct even if the explicit severity/subtype label is missing from the diagnostic statement.
        The key question is: did the agent identify the same underlying disease or condition, and is the management appropriate? If yes, award the point. Only mark a diagnosis as WRONG when the agent names a fundamentally different condition (e.g. "allergic conjunctivitis" when the expected answer is "bacterial conjunctivitis") or gets a clinically significant classification wrong (e.g. "no dehydration" when the case clearly shows "some dehydration").
...
async def run_oneday_scenario(test_scenario: Scenario, testrun_uid: str, model_id: str, diagnosis_only: bool = False, use_turn: bool = False, turn_uuid: str | None = None, request=None):
    ...
    nurse_description = (
        "You are a nurse texting an AI medical diagnosis agent on WhatsApp. "
        "This is a test — you are evaluating whether the agent can ask the right questions and "
        "reach the correct diagnosis by following its medical guidelines. "
        "You have a patient chart with symptoms, vitals, and test results. "
        "The agent doesn't know what's wrong yet — it needs to ask you questions to figure it out.\n\n"
        "## FIRST MESSAGE\n\n"
        "Your very first message must be ONLY the word:\n\n"
        "Hello\n\n"
        "Nothing else. No symptoms, no questions, no context. Just that single word. "
        "Wait for the agent to respond before saying anything about the patient.\n\n"
        "## How to behave after the first message\n\n"
        "- After the agent greets you, briefly mention the patient's main complaint — just the age "
        "and the primary symptom in one short sentence (e.g. 'I have a 4-year-old boy with diarrhea' "
        "or 'I'm seeing a man with chest pain'). Do NOT include vitals, test results, negatives, or "
        "secondary symptoms yet.\n"
        "- Let the agent lead. Wait for it to ask questions, then answer only what was asked. Don't "
        "volunteer extra information the agent hasn't asked about.\n"
        "- Be brief and natural. You're texting on WhatsApp, not writing a medical report. Short "
        "sentences, no bullet points, no structured formats.\n"
        "- If the agent asks multiple questions in one message, answer all of them — but keep each "
        "answer short.\n"
        "- If the agent asks about something not in your chart, say you don't have that information "
        "or the test wasn't done, and ask them to work with what you have.\n"
        "- Never give the agent your own diagnostic opinions or suggest what the diagnosis might be.\n"
        "- Never ask the agent to perform physical exams or tests — everything has already been done.\n"
        "- If the agent offers to send a document, checklist, or PDF, just say you're done and want "
